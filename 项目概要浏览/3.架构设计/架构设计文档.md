...
> **v1.0.0 发行冻结说明（2026-07-15）：** 正式发行只交付五服务基础方案：MySQL 8、Redis 7、Spring Boot 后端、Vue/Nginx 前端、Python ETL worker。报告格式为 PDF-only；Word/Excel、Kafka、HDFS、Spark、Hive 和 WebSocket 告警均不进入本次发行。部署链路以 `docker-compose.yml`、版本化 `sql/migrations/*.sql`、`schema_migrations`、报告持久化卷和 GitHub Actions 验收脚本为准；旧文档中的进阶组件仅作为后续规划，不作为 v1.0 验收条件。
