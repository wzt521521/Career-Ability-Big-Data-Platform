...
`.env` 只用于本地运行，不得提交。生产环境必须覆盖 `MYSQL_ROOT_PASSWORD`、`MYSQL_APP_PASSWORD`、`MYSQL_MIGRATOR_PASSWORD`、`REDIS_PASSWORD` 和 `JWT_SECRET`；JWT 密钥至少为 32 字节随机字符串。仓库中的默认值仅用于本地开发。

## 3. 本地启动

先启动 MySQL 和 Redis。生产 compose 不暴露 MySQL/Redis 到宿主机；本地调试需要叠加开发 override：
...
$env:SPRING_DATASOURCE_USERNAME = "career_app"
$env:SPRING_DATASOURCE_PASSWORD = "local-development-app-password"
$env:SPRING_REDIS_PASSWORD = "local-development-redis-password"
...
不再提供仓库内默认管理员或默认密码。首次部署时可一次性设置 `INITIAL_ADMIN_USERNAME` 和至少 12 字符的 `INITIAL_ADMIN_PASSWORD`；用户创建成功后保留账号并删除该密码环境变量。生产 profile 必须设置至少 32 字节且非开发默认值的 `JWT_SECRET`，否则后端会拒绝启动。
...
Compose 保留 `mysql`、`redis`、`db-migrate`、`backend`、`frontend` 和 `python-etl` 服务。前端地址为 `http://localhost`，后端本机回环地址为 `http://127.0.0.1:8080`。生产 compose 不暴露 MySQL/Redis；如需本机连接数据库或缓存，使用 `docker-compose.dev.yml`。

`sql/init.sql` 只会在空 MySQL 数据卷首次创建时执行，也会重建全部表，不能用于已有数据库升级。已有数据库部署本版本时使用版本化迁移服务：

```powershell
docker compose run --rm db-migrate
```

迁移状态写入 `schema_migrations`，脚本 checksum 改变或包含 `DROP TABLE` 时会中止。

完整运维、备份恢复和故障排查见 `docs/operations.md`。
...
Postman 集合位于 `docs/postman/黄健熙-认证与开放API.postman_collection.json`；发行验证集合位于 `docs/postman/release-core-api.postman_collection.json`，覆盖采集、分析、画像、推荐、报告下载和双认证开放接口。运行后端后可通过 `curl http://127.0.0.1:8080/v3/api-docs -o openapi.json` 导出 OpenAPI JSON；CI 会上传该发行工件。
...
- `docs/demo-script.md`
- `docs/data-and-third-party-notices.md`
